...
# from src.off_chain.historical_data.cow_endpoint_surplus import get_surplus_difference
...
class TestSurplusCalculation(unittest.TestCase):
    ...
    def test_hash_input(self):
        ...
        self.hash = "0xeace63119d7f8477c3d826efb178b70eb7ef37cc084b86ef7ceedbd230270e0f"
        self.file_name = str(hash) + ".txt"
        ...
        self.assertIsNone(
            instance.get_surplus_by_input(settlement_hash=hash)
        )

    def test_block_range_input(self):
        # Test that function works with a start and end block input
        self.start_block = 16935510
        self.end_block = 16936190
        self.file_name = str(self.start_block) + "_surplusTo_" + str(self.end_block) + ".txt"
    ...
    def test_surplus_difference(self):
        ...
        (self.diff_surplus, self.percent_deviation, self.surplus_token) = get_surplus_difference(self.order_data, self.clearingPrices, self.order)
        self.assertEqual(
            self.diff_surplus, -123572507, "Not equivalent!"
        )
